...
final class SparkleTests: XCTestCase {
    func testInfoPlistUsesGitHubLatestAppcastAndManualInstall() throws {
        ...
        XCTAssertEqual(plist["SUFeedURL"] as? String,
                       "https://github.com/bay2/ui_review_tools/releases/latest/download/appcast.xml")
    ...
    func testPublishReleaseDryRunUsesVersionedGitHubAssets() throws {
        ...
        XCTAssertTrue(result.out.contains("--repo bay2/ui_review_tools"), result.out)
        ...
        XCTAssertTrue(result.out.contains("\"\(root.path)/build/UI Review-\(version).pkg\""), result.out)
